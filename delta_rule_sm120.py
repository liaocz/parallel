...
_DELTA_RULE_COMPILED_CACHE = {}
_DELTA_RULE_DUMMY_CACHE = {}
_DELTA_RULE_TENSORMAPS_CACHE = {}
...
def _cached_dummy_tensors(device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
    key = _cuda_device_index(device)
    cached = _DELTA_RULE_DUMMY_CACHE.get(key)
    if cached is None:
        cached = (
            torch.empty(1, dtype=torch.float32, device=device),
            torch.empty(1, dtype=torch.int64, device=device),
        )
        _DELTA_RULE_DUMMY_CACHE[key] = cached
    return cached


def _cached_tensormaps(
    device: torch.device,
    stream_val: int,
    sm_count: int,
) -> torch.Tensor:
    key = (_cuda_device_index(device), int(stream_val), int(sm_count))
    cached = _DELTA_RULE_TENSORMAPS_CACHE.get(key)
    if cached is None:
        cached = torch.empty(sm_count * 128, dtype=torch.uint8, device=device)
        _DELTA_RULE_TENSORMAPS_CACHE[key] = cached
    return cached


def _compiled_delta_rule_kernel(
    needs_alpha: bool,
    needs_beta: bool,
    needs_init_state: bool,
    needs_checkpointing: bool,
    split_v_parts: int,
    aligned_full_blocks: bool,
    ...
    key = (
        bool(needs_alpha),
        bool(needs_beta),
        bool(needs_init_state),
        bool(needs_checkpointing),
        int(split_v_parts),
        bool(aligned_full_blocks),
        kernel_dtype,
    )
    compiled = _DELTA_RULE_COMPILED_CACHE.get(key)
    if compiled is None:
        ...
        compiled = cached_compile(
            delta_rule_kernel,
            *kernel_args,
            compile_options=_SM120A_COMPILE_OPTIONS,
        )
        _DELTA_RULE_COMPILED_CACHE[key] = compiled
    return compiled
...
class _FullyFusedDeltaRuleSm120(KeyedCompileMixin):
    ...
    def __init__(
        ...
        needs_checkpointing: bool,
        split_v_parts: int,
        aligned_full_blocks: bool = False,
        ...
        self.split_v_parts    = split_v_parts
        self.aligned_full_blocks = aligned_full_blocks
        self.dtype            = dtype
        ...
        self.BLK_KV          = 64
        self.D               = 128
        self.V_TILE          = self.D // self.split_v_parts
        self.v_mma_warps     = 8 // self.split_v_parts
        self.q_stage          = 1
        self.k_stage          = 2
        self.v_stage          = 1
        self.o_stage          = 1
        self.alpha_beta_stage = 2
        self.alpha_beta_layout = "head_major"
        self.kernel_variant = "v72-aligned-ldst-loop-splitv2"
    ...
    def get_next_work(
        ...
        split_v_parts = cutlass.Int32(self.split_v_parts)
        logical_bx    = bx // split_v_parts
        split_v_idx   = bx - logical_bx * split_v_parts
        seq_idx       = logical_bx // num_sab_heads
        o_head_idx    = logical_bx % num_sab_heads
        ...
        v_row_offset = split_v_idx * cutlass.Int32(self.V_TILE)
        ...
        return WorkDesc(
            ...
            split_v_idx=split_v_idx,
            v_row_offset=v_row_offset,
    ...
    @cute.jit
    def _math_order_init_static(self, is_kk_role: bool):
        if cutlass.const_expr(not is_kk_role):
            cute.arch.barrier_arrive(
                barrier_id=NamedBarrier.MATH_WG0, number_of_threads=256)

    @cute.jit
    def _math_order_wait_static(self, is_kk_role: bool):
        if cutlass.const_expr(is_kk_role):
            cute.arch.barrier(
                barrier_id=NamedBarrier.MATH_WG0, number_of_threads=256)
        else:
            cute.arch.barrier(
    ...
    @cute.jit
    def _math_order_notify_static(self, is_kk_role: bool):
        if cutlass.const_expr(is_kk_role):
            cute.arch.barrier_arrive(
                barrier_id=NamedBarrier.MATH_WG1, number_of_threads=256)
        else:
    ...
    def kv_decay_v(
        ...
        if cutlass.const_expr(self.needs_alpha):
            alpha_cumlog = sAlpha[None, AlphaProcessor.CUMSUM_LOG, alpha_stage]
            block_last = B - cutlass.Int32(1)
            if cutlass.const_expr(not is_final_block):
                block_last = cutlass.Int32(self.BLK_KV - 1)
            block_log    = cutlass.Float32(alpha_cumlog[block_last])
    ...
    def load_qkv_tma(
        ...
        mV = cute.domain_offset(
            (v_row_offset, blk_tok),
            tma_tensor_v[None, None, v_head_idx],
        )
        gV = cute.zipped_divide(mV, (self.V_TILE, self.BLK_KV))[((None, None), (cutlass.Int32(0), cutlass.Int32(0)))]
    ...
    def load_beta(
        ...
        for i in cutlass.range_constexpr(num_iters):
            ...
            tok = blk_tok + row
            if tok < tok_end:
                sBeta_k[row] = g_beta[sab_head_idx * total_seqlen + tok]
            else:
                sBeta_k[row] = cutlass.Float32(0.0)

    @cute.jit
    def load_beta_full_block(
        self,
        sBeta:        cute.Tensor,
        g_beta:       cute.Tensor,
        blk_tok:      cutlass.Int32,
    ...
    def kv_store(
        ...
        for i in cutlass.range_constexpr(cute.size(tKVrKV)):
            v_idx, k_idx = tKVcKV[i]
            gKV[k_idx, v_row_offset + v_idx] = tKVrKV[i]
    ...
    def maybe_store_checkpoint(
        ...
        if cutlass.const_expr(self.needs_checkpointing):
            if block_end <= seq_len and block_end % checkpoint_every_n_tokens == cutlass.Int32(0):
                ...
                self.kv_store(tKVrKV, gCheckpointKV, kv_thr_mma, v_row_offset)
    ...
    def compute_loop_body(
        ...
        v_tile = cute.size(sV_DS, mode=[0])
        tile_shape_qk = (blk_q, blk_kv, d)
        tile_shape_kk = tile_shape_qk
        tile_shape_o1 = (v_tile, blk_q, d)
        tile_shape_o2 = (v_tile, blk_q, blk_kv)
        tile_shape_sk = (v_tile, blk_kv, d)
        tile_shape_newv = (v_tile, blk_kv, blk_kv)
        ...
        v_thread_idx = thread_idx
        if cutlass.const_expr(self.split_v_parts == 2):
            v_thread_idx = kk_thread_idx
        do_v_math = wg_idx == wg_idx
        if cutlass.const_expr(self.split_v_parts == 2):
            do_v_math = is_kk_role

        # O1/O2/SK/NewV: 8 warps for 128 rows, 4 warps for a 64-row V split.
        o1_tiled_mma = cute.make_tiled_mma(
            mma_atom_8w, cute.make_layout((self.v_mma_warps, 1, 1)),
        ...
        sk_thr_mma   = sk_tiled_mma.get_slice(v_thread_idx)
        newv_thr_mma = newv_tiled_mma.get_slice(v_thread_idx)
        o1_thr_mma   = o1_tiled_mma.get_slice(v_thread_idx)
        o2_thr_mma   = o2_tiled_mma.get_slice(v_thread_idx)
        kv_thr_mma   = kv_tiled_mma.get_slice(v_thread_idx)
        ...
        sk_thr_copy_B = sk_tiled_copy_B.get_slice(v_thread_idx)
        sk_thr_copy_C = sk_tiled_copy_C.get_slice(v_thread_idx)
        ...
        newv_thr_copy_B = newv_tiled_copy_B.get_slice(v_thread_idx)
        ...
        kv_thr_copy_B = kv_tiled_copy_B.get_slice(v_thread_idx)
        ...
        o1_thr_copy_B = o1_tiled_copy_B.get_slice(v_thread_idx)
        o2_thr_copy_B = o2_tiled_copy_B.get_slice(v_thread_idx)
        ...
        o_thr_copy_r2s = o_tiled_copy_r2s.get_slice(v_thread_idx)
        ...
        cO      = cute.make_identity_tensor((v_tile, blk_q))
        tOcO    = o1_thr_mma.partition_C(cO)
        cSK     = cute.make_identity_tensor((v_tile, blk_kv))
        tSKcSK  = sk_thr_mma.partition_C(cSK)
        cV      = cute.make_identity_tensor((v_tile, blk_kv))
        ...
        tOrO = cute.make_rmem_tensor(
            o1_thr_mma.partition_shape_C((v_tile, blk_q)), self.acc_dtype)
        ...
        tSKrSK = cute.make_rmem_tensor(
            sk_thr_mma.partition_shape_C((v_tile, blk_kv)), self.acc_dtype)
        ...
        tNewVrC = cute.make_rmem_tensor(
            newv_thr_mma.partition_shape_C((v_tile, blk_kv)), self.acc_dtype)
        if cutlass.const_expr(self.split_v_parts == 2):
            self._math_order_wait_static(is_kk_role)
        else:
        ...
        if cutlass.const_expr(self.split_v_parts == 2):
            self._math_order_notify_static(is_kk_role)
        else:
        ...
        if cutlass.const_expr(self.needs_alpha):
            ...
            block_coeff = cutlass.Float32(
                sAlpha[block_last, AlphaProcessor.CUMPROD, alpha_stage])
    ...
    def run_load_qkv_role(
        ...
        for blk in cutlass.range(num_blocks, unroll=1):
            (
                ...
                blk, tok_start,
                q_head_idx, k_head_idx, v_head_idx, v_row_offset,
            )
    ...
    def run_load_alpha_role(
        ...
        for blk in cutlass.range(num_blocks, unroll=1):
            ...
            if cutlass.const_expr(self.needs_alpha):
                ...
                cute.arch.fence_view_async_shared()
                if cutlass.const_expr(self.aligned_full_blocks):
                    self.load_alpha_full_block(
                        sAlpha, g_alpha, blk_tok, sab_head_idx,
                        total_seqlen, alpha_producer_state.index,
                    )
                else:
                    self.load_alpha(
    ...
    def run_load_beta_role(
        ...
        for blk in cutlass.range(num_blocks, unroll=1):
            ...
            if cutlass.const_expr(self.needs_beta):
                ...
                cute.arch.fence_view_async_shared()
                if cutlass.const_expr(self.aligned_full_blocks):
                    self.load_beta_full_block(
                        sBeta, g_beta, blk_tok, sab_head_idx,
                        total_seqlen, beta_producer_state.index,
                    )
                else:
                    self.load_beta(
    ...
    def run_math_role(
        ...
        if cutlass.const_expr(self.split_v_parts == 2):
            self._math_order_init_static(is_kk_role)
        else:
        ...
        kv_tiled_mma = cute.make_tiled_mma(
            ...
            cute.make_layout((self.v_mma_warps, 1, 1)),
            permutation_mnk=(self.V_TILE, self.D, self.BLK_KV))
        v_math_tidx = math_tidx
        if cutlass.const_expr(self.split_v_parts == 2):
            v_math_tidx = math_tidx % cutlass.Int32(128)
        do_v_math = wg_idx == wg_idx
        if cutlass.const_expr(self.split_v_parts == 2):
            do_v_math = wg_idx == MathWarpGroupRole.KK
        kv_thr_mma = kv_tiled_mma.get_slice(v_math_tidx)
        tKVrKV = cute.make_rmem_tensor(
            kv_thr_mma.partition_shape_C((self.V_TILE, self.D)), self.acc_dtype)
        ...
        if cutlass.const_expr(self.needs_init_state):
            ...
            if do_v_math:
                self.kv_load(tKVrKV, gInitKV, kv_thr_mma, work_desc.v_row_offset)

        first_B = work_desc.seq_len
        if first_B > cutlass.Int32(self.BLK_KV):
            first_B = cutlass.Int32(self.BLK_KV)
        first_is_final_block = True
        first_loop_B = first_B
        if cutlass.const_expr(self.aligned_full_blocks):
            first_is_final_block = False
            first_loop_B = cutlass.Int32(self.BLK_KV)
        if cutlass.const_expr(self.needs_init_state):
            (
                q_consumer_state,
                k_consumer_state,
                ...
                False, first_is_final_block, first_loop_B,
                tKVrKV, scale, wg_idx, is_kk_role,
            )
        else:
            (
                q_consumer_state,
                ...
                alpha_pipeline, alpha_consumer_state,
                beta_pipeline, beta_consumer_state,
                True, first_is_final_block, first_loop_B,
                tKVrKV, scale, wg_idx, is_kk_role,
            )
        if do_v_math:
        ...
        regular_end_blk = num_blocks - cutlass.Int32(1)
        if cutlass.const_expr(self.aligned_full_blocks):
            regular_end_blk = num_blocks
        for blk in cutlass.range(cutlass.Int32(1), regular_end_blk, cutlass.Int32(1), unroll=1):
            (
                q_consumer_state,
                k_consumer_state,
                v_consumer_state,
        ...
        if cutlass.const_expr(not self.aligned_full_blocks):
        ...
        if do_v_math:
            self.kv_store(tKVrKV, gStateKV, kv_thr_mma, work_desc.v_row_offset)
    ...
    def __call__(
        ...
        v_storage_layout_sd = cute.coalesce(
            cute.tile_to_shape(
                qkv_smem_layout_atom,
                (self.BLK_KV, self.V_TILE, self.v_stage),
                order=(0, 1, 2),
            ),
            target_profile=(1, 1, 1),
        )
        ...
        o_storage_layout = cute.tile_to_shape(
            o_smem_layout_atom,
            (self.V_TILE, self.BLK_Q, self.o_stage),
            order=(1, 0, 2),
        )
        ...
        tma_atom_v, tma_tensor_v = cpasync.make_tiled_tma_atom(
            tma_load_op, g_v, v_smem_layout, (self.V_TILE, self.BLK_KV))

        tma_store_op = cpasync.CopyBulkTensorTileS2GOp()
        tma_atom_o, tma_tensor_o = cpasync.make_tiled_tma_atom(
            tma_store_op, g_o, o_smem_layout, (self.V_TILE, self.BLK_Q))
        ...
        self.atrex_gdn_delta_rule_sm120_kernel(
            g_alpha, g_beta,
            tma_atom_q, tma_tensor_q,
            tma_atom_k, tma_tensor_k,
            tma_atom_v, tma_tensor_v,
            ...
            scale, num_q_heads, num_k_heads, num_v_heads,
            num_sab_heads, total_seqlen, num_seqs,
    ...
    @cute.kernel
    def atrex_gdn_delta_rule_sm120_kernel(
        self,
        g_alpha:      cute.Tensor,
        g_beta:       cute.Tensor,
        tma_atom_q:   cute.CopyAtom,
        ...
        if warp_group_idx == WarpGroupRole.LDST:
            ...
            if ldst_warp_role == LoadStoreWarpRole.LOAD_QKV:
                ...
                CollectiveStoreTma(
                    self.BLK_Q, self.V_TILE, self.aligned_full_blocks).run(
                ...
                self.run_load_beta_role(
                    ...
                    work_desc.o_head_idx(num_q_heads, num_v_heads), total_seqlen,
                )
                ...
                self.run_load_alpha_role(
                    ...
                    tok_end, work_desc.o_head_idx(num_q_heads, num_v_heads), total_seqlen,
                )
        else:
            cute.arch.setmaxregister_increase(mma_registers)
            if cutlass.const_expr(self.split_v_parts == 2):
                if warp_group_idx == WarpGroupRole.MATH_KK:
                    self.run_math_role(
                        sQ_SD, sK_SD, sK_DS, sV_DS, sQK, sKK_inv, sKK_opd, sO, sAlpha, sBeta,
                        q_pipeline, k_pipeline, v_pipeline,
...
def delta_rule_prefill_dsl(
    ...
    split_v_parts: int = 1,
    aligned_full_blocks: bool | None = None,
):
    ...
    if split_v_parts not in (1, 2):
        raise ValueError(f"split_v_parts must be 1 or 2, got {split_v_parts}")
    if D % split_v_parts != 0:
        raise ValueError(
            f"D={D} must be divisible by split_v_parts={split_v_parts}")
    ...
    if needs_checkpointing:
        ...
        if state_checkpoints.ndim != 4:
            raise ValueError(
                "state_checkpoints must be 4D "
                "[total_checkpoints, num_sab_heads, D, D]")
        if tuple(state_checkpoints.shape[1:]) != (num_sab_heads, D, D):
            raise ValueError(
                "state_checkpoints shape mismatch: expected "
                f"[total_checkpoints, {num_sab_heads}, {D}, {D}], got "
                f"{list(state_checkpoints.shape)}")
        if checkpoint_cu_starts.dtype != torch.int64:
            raise ValueError(
                f"checkpoint_cu_starts must be int64, got {checkpoint_cu_starts.dtype}")
        if checkpoint_cu_starts.ndim != 1 or checkpoint_cu_starts.numel() != num_seqs + 1:
            raise ValueError(
                "checkpoint_cu_starts must be 1D [num_seqs + 1], got "
                f"shape={list(checkpoint_cu_starts.shape)}")
        if int(checkpoint_cu_starts[-1].item()) != int(state_checkpoints.shape[0]):
            raise ValueError(
                "checkpoint_cu_starts[-1] must match state_checkpoints.shape[0]")
    ...
    if aligned_full_blocks is None:
        aligned_full_blocks = (
            num_seqs == 1 and total_seqlen % 64 == 0
        )
    else:
        aligned_full_blocks = bool(aligned_full_blocks)
    ...
    _dummy_f32, _dummy_i64 = _cached_dummy_tensors(q.device)
    alpha_t      = alpha.transpose(0, 1).contiguous() if needs_alpha else _dummy_f32
    beta_t       = beta.transpose(0, 1).contiguous()  if needs_beta  else _dummy_f32
    ...
    checkpoint_cu_t = (
        ...
        if needs_checkpointing else _dummy_i64
    )
    ...
    tensormaps_t = _cached_tensormaps(q.device, int(stream_val), sm_count)

    # Keep head counts and varlen extents runtime values across cached compiles.
    q_cute = from_dlpack(q_tma, assumed_align=128).mark_layout_dynamic(leading_dim=1)
    k_cute = from_dlpack(k_tma, assumed_align=128).mark_layout_dynamic(leading_dim=0)
    v_cute = from_dlpack(v_tma, assumed_align=128).mark_layout_dynamic(leading_dim=0)
    o_cute = from_dlpack(o_tma, assumed_align=128).mark_layout_dynamic(leading_dim=0)
    alpha_cute = from_dlpack(alpha_t.reshape(-1), assumed_align=128).mark_layout_dynamic()
    beta_cute = from_dlpack(beta_t.reshape(-1), assumed_align=128).mark_layout_dynamic()
    state_cute = from_dlpack(state.reshape(-1), assumed_align=128).mark_layout_dynamic()
    init_state_cute = from_dlpack(
        init_state_t.reshape(-1), assumed_align=128
    ).mark_layout_dynamic()
    state_checkpoints_cute = from_dlpack(
        state_checkpoints_t.reshape(-1), assumed_align=128
    ).mark_layout_dynamic()
    checkpoint_cu_cute = from_dlpack(
        checkpoint_cu_t, assumed_align=8).mark_layout_dynamic()
    ...
    kernel_args = (
        ...
        num_seqs * num_sab_heads * split_v_parts,
        stream,
    )
    compiled_delta_rule_kernel = _compiled_delta_rule_kernel(
        needs_alpha,
        needs_beta,
        needs_init_state,
        needs_checkpointing,
