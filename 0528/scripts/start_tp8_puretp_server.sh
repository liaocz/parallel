...
rank="${1:?usage: start_tp8_puretp_server.sh NODE_RANK [DIST_PORT] [MODE]}"
dist_port="${2:-29540}"
mode="${3:-tp8_puretp_nvl_cg16}"
root_dir="/home/admin/0528"
log_path="${root_dir}/server_logs/${mode}_rank${rank}_server.log"
cmd_path="${root_dir}/commands/${mode}_rank${rank}_server_command.sh"
...
export EXTRA_SERVER_ARGS="${extra_args}"
...
{
  echo "=== ${mode} rank ${rank} server start ==="
