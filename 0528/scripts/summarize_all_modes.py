...
def write_report(results, summary_csv, comparison_csv):
    ...
    lines.append("")
    lines.append("## 后续 profiling 建议")
    lines.append("")
    lines.append("按照当前结果，TP8 相比 TP4 性能较差，满足继续 profiling 的条件。优先选择 `32k_500_c1` 和 `128k_500_c1`，因为二者最能暴露 TTFT/prefill 问题；再选择 `32k_500_c64` 观察高并发下 TP8 接近 TP4 的原因。")
    lines.append("")
    lines.append("## 容器启动命令")
    lines.append("")
