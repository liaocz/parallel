...
def atrex_fused_chunk_h_kernel(
    ...
    gK_full = cute.make_tensor(mKnorm.iterator.align(128), mKnorm.layout)[(i_b, i_h, None, None)]
    gQ_full = cute.make_tensor(mQnorm.iterator.align(128), mQnorm.layout)[(i_b, i_h, None, None)]
    thr_cp = tiled_copy_kq.get_slice(tidx)
    thr_sK_cp = thr_cp.partition_D(sK)
    thr_sQ_cp = thr_cp.partition_D(sQ)
...
def _run_3kernel_v31_final_state(
    ...
    mK = from_dlpack(kc, assumed_align=16)
    mV = from_dlpack(vc, assumed_align=16)
    mG = from_dlpack(gc, assumed_align=16)
    mBeta = from_dlpack(betac, assumed_align=16)
    if workspace is None:
        mQnorm = from_dlpack(q_norm, assumed_align=16)
        mKnorm = from_dlpack(k_norm, assumed_align=16)
    ...
    mO = from_dlpack(o, assumed_align=16)
    mFinalState = from_dlpack(final_state, assumed_align=16)
...
def _alloc_3kernel_v31_workspace(
    ...
    workspace["mGQK"] = from_dlpack(workspace["gated_qk"], assumed_align=16)
    workspace["mExpGC"] = from_dlpack(workspace["exp_gc"], assumed_align=16)
    workspace["mExpDecay"] = from_dlpack(workspace["exp_decay"], assumed_align=16)
    if not fused_tail_k0inv:
        workspace["mKK"] = from_dlpack(workspace["kk"], assumed_align=16)
        workspace["mQK"] = from_dlpack(workspace["qk"], assumed_align=16)
    return workspace
