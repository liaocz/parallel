...
rank="${1:?usage: start_tp8_dp8_dpa_megamoe_tune.sh NODE_RANK [VARIANT] [CHUNKED_PREFILL] [DIST_PORT] [EXTRA_ARGS...]}"
variant="${2:-tp8_dp8_dpa_megamoe_tune}"
...
run_dir="${root_dir}/tune_runs/${variant}"
metrics_dir="${root_dir}/metrics/${variant}"
log_path="${run_dir}/rank${rank}.server.log"

mkdir -p "$run_dir" "$metrics_dir" /home/admin/sglang_runs
rm -f "$log_path"
